...
- 只有通过内置 Claude Code ACP preset 使用 `npx` 时才需要 Node.js/npm。
...
如果使用 Claude Code，可以直接安装内置 ACP preset：
...
这个 preset 会注册 `npx -y @zed-industries/claude-agent-acp`，并记录人工 TUI 接管模板：

```bash
cd {cwd} && claude --resume {session_id}
```

ACP 分发完成后，`task status`、`task resume`、任务看板和生成的 brief 都会展示具体恢复命令，例如：

```bash
cd /path/to/project && claude --resume 174041b8-784e-4e7a-82a6-b0c0c06d6193
```

当人类需要进入同一个子 Agent 对话时，直接在终端运行这条命令。自定义 ACP
注册也可以通过 `--resume-template "cd {cwd} && /path/to/custom-claude --resume {session_id}"`
启用同样的接管命令。
