...
- Use `tmux has-session -t <session>` through the configured `HELM_AGENT_TMUX_BIN`.
...
- If a recorded session exists, set `running` for tasks in `queued`, `running`, or `blocked`, clear the blocker, and record a `sync_alive` event.
